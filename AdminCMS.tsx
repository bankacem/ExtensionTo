...
const AdminCMS: React.FC<AdminCMSProps> = ({ onExit }) => {
  ...
  const generateAIDraft = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Act as a professional technology writer. Write a comprehensive blog post for our directory about: "${formData.title}". 
                  The tone should be sophisticated and informative. 
                  Format the response as pure HTML using only <h2>, <p>, and <ul> tags. 
                  Also, provide a catchy 150-character SEO summary.
                  Return the result in JSON format with keys "content" and "excerpt".`,
        config: { 
  ...
  const boostSEO = () => {
    ...
    const keywords = (formData.category || 'browser, extension, privacy, security').toLowerCase();
  ...
  const handleMediaUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files) return;
    // Fix: Explicitly cast Array.from(files) to File[] to ensure the 'file' variable is not of type 'unknown'.
    // This resolves errors when accessing file.name, file.type, and passing file to readAsDataURL.
    ...
    return (
      <div className="min-h-screen bg-[#F2F2F7] flex items-center justify-center p-6 font-sans">
        <div className="w-full max-w-md bg-white rounded-[40px] shadow-2xl p-12 border border-slate-200/50">
          ...
          <p className="text-slate-500 text-center mb-10 font-medium">Platform Management Portal</p>
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 mb-2 block">Username</label>
              <input type="text" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-4 ring-blue-50 font-bold transition-all" value={loginForm.username} onChange={e => setLoginForm({...loginForm, username: e.target.value})} required />
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 mb-2 block">Password</label>
              <input type="password" className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-4 ring-blue-50 font-bold transition-all" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} required />
            </div>
    ...
    <div className={`min-h-screen flex ${bgClass} font-sans transition-colors duration-500`}>
      ...
      <aside className={`w-80 fixed h-full border-r ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} z-50 flex flex-col shadow-2xl`}>
        <div className="p-10 border-b border-inherit flex items-center gap-4">
          ...
          <div>
            ...
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Enterprise Hub</span>
          </div>
        </div>
        ...
        <div className="p-8 border-t border-inherit space-y-4">
          ...
          <button onClick={handleLogout} className="w-full py-4 bg-rose-50 text-rose-600 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all">LOGOUT SESSION</button>
        </div>
      </aside>
      ...
      <main className="ml-80 flex-grow p-12 overflow-y-auto">
          ...
          <div className="space-y-12 animate-in fade-in duration-700">
            <header className="flex justify-between items-end">
              <div>
                ...
                <p className="text-slate-500 text-xl font-medium">Real-time performance metrics and content flow.</p>
              </div>
              <div className="flex gap-4">
                 <button onClick={handleExport} className="px-8 py-5 bg-white border border-slate-200 text-slate-900 rounded-[24px] font-black text-sm shadow-sm hover:bg-slate-50 transition-all">BACKUP SITE</button>
                 <button onClick={() => {setEditMode('post'); setFormData({title: '', content: ''}); setIsEditing(true);}} className="px-8 py-5 bg-blue-600 text-white rounded-[24px] font-black text-sm shadow-2xl shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95">NEW ARTICLE</button>
              </div>
            </header>

            <div className="grid grid-cols-4 gap-8">
              {[
                { label: 'Platform Reach', val: dashboardStats.totalViews, icon: '👁️', color: 'text-blue-600' },
                { label: 'Journal Articles', val: dashboardStats.postsCount, icon: '📄', color: 'text-indigo-600' },
                { label: 'Active Extensions', val: dashboardStats.extsCount, icon: '📦', color: 'text-emerald-600' },
                { label: 'Conversion Rate', val: dashboardStats.totalInstalls, icon: '🚀', color: 'text-rose-600' },
              ].map((s, i) => (
        ...
        {activeTab === 'posts' && !isEditing && (
          <div className="space-y-12 animate-in slide-in-from-right-12 duration-500">
             <header className="flex justify-between items-center">
               ...
               <div className="flex gap-4">
                 <input type="text" placeholder="Filter articles..." className={`px-6 py-4 rounded-2xl border outline-none focus:ring-4 ring-blue-50 font-bold ${inputClass}`} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                 <button onClick={() => {setEditMode('post'); setFormData({title: ''}); setIsEditing(true);}} className="px-10 py-5 bg-blue-600 text-white rounded-[24px] font-black text-sm shadow-xl shadow-blue-100">CREATE POST</button>
               </div>
             </header>
                 ...
                 <div key={post.id} className={`p-8 rounded-[48px] border shadow-sm flex gap-8 group transition-all hover:border-blue-500/30 ${cardClass}`}>
                    ...
                    <div className="flex flex-col justify-between flex-grow">
                       ...
                       <div className="flex justify-between items-center">
                          ...
                          <div className="flex gap-4">
                             <button onClick={() => {setFormData(post); setEditMode('post'); setIsEditing(true);}} className="text-[10px] font-black text-slate-400 hover:text-blue-600 uppercase tracking-widest transition-colors">Edit</button>
                             <button onClick={() => { if(window.confirm('Delete this post?')) setPosts(posts.filter(p => p.id !== post.id)); }} className="text-[10px] font-black text-rose-500 hover:text-rose-600 uppercase tracking-widest transition-colors">Delete</button>
                          </div>
                       </div>
                    </div>
                 </div>
          ...
          <div className="max-w-6xl animate-in slide-in-from-bottom-12 duration-700">
            <header className="flex justify-between items-center mb-12">
               <button onClick={() => setIsEditing(false)} className="text-sm font-black text-slate-400 hover:text-blue-600 transition-colors flex items-center gap-3">
                  <span className="text-xl">←</span> DISCARD CHANGES
               </button>
               <div className="flex gap-4">
                  {editMode === 'post' && (
                    <>
                      ...
                      <button onClick={boostSEO} className="px-8 py-4 bg-emerald-600 text-white rounded-[20px] font-black text-xs shadow-xl shadow-emerald-100 hover:scale-105 transition-all">🚀 SEO BOOST</button>
                    </>
                  )}
                  <button onClick={handleSave} className="px-12 py-5 bg-blue-600 text-white rounded-[24px] font-black text-sm shadow-2xl shadow-blue-200 active:scale-[0.98]">COMMIT CHANGES</button>
               </div>
            </header>
              ...
              <div className="grid grid-cols-2 gap-12">
                 <div className="space-y-8">
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Category / Tagging</label>
                 ...
                 <div className="space-y-8">
                    <div className="space-y-3">
                      ...
                      <textarea className={`w-full p-6 rounded-[24px] border font-medium text-sm h-32 leading-relaxed ${inputClass}`} placeholder="Search engine description..." value={formData.seoDesc} onChange={e => setFormData({...formData, seoDesc: e.target.value})} />
                    </div>
                 </div>
              </div>

              <div className="space-y-4">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Content Core (HTML Support)</label>
                <textarea 
        ...
        {activeTab === 'media' && (
          <div className="space-y-12 animate-in fade-in duration-500">
            <header className="flex justify-between items-center">
              ...
              <button onClick={() => fileInputRef.current?.click()} className="px-10 py-5 bg-blue-600 text-white rounded-[24px] font-black text-sm shadow-xl shadow-blue-100">UPLOAD TO LIBRARY</button>
            </header>
            <input type="file" ref={fileInputRef} className="hidden" multiple onChange={handleMediaUpload} />
            
            <div className="grid grid-cols-4 gap-8">
              {mediaLibrary.map(item => (
                <div key={item.id} className={`p-4 rounded-[40px] border shadow-sm group relative overflow-hidden transition-all hover:border-blue-500 ${cardClass}`}>
                   ...
                   <button 
                    onClick={() => { if(window.confirm('Delete media?')) { const updated = mediaLibrary.filter(m => m.id !== item.id); setMediaLibrary(updated); localStorage.setItem(MEDIA_KEY, JSON.stringify(updated)); } }} 
        ...
        {activeTab === 'analytics' && (
           <div className="space-y-12 animate-in fade-in duration-500">
             <header>
                <h1 className="text-5xl font-black tracking-tighter mb-4">Intelligence Lab</h1>
                <p className="text-slate-500 text-xl font-medium">Holistic view of platform health and conversion analytics.</p>
             </header>
             <div className="grid grid-cols-3 gap-8">
                <div className={`col-span-2 p-12 rounded-[64px] border shadow-2xl ${cardClass}`}>
                   <h3 className="text-xl font-black mb-10 text-slate-400 uppercase tracking-widest">Regional Velocity</h3>
                   <div className="space-y-8">
                      {['North America', 'Western Europe', 'South Asia', 'Australasia'].map((region, i) => (
                        <div key={region}>
                           <div className="flex justify-between font-bold mb-3"><span>{region}</span><span>{90 - (i * 12)}%</span></div>
                           <div className="h-3 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                              <div className="h-full bg-blue-600 rounded-full transition-all duration-1000" style={{width: `${90 - (i * 12)}%`}}></div>
                           </div>
                        </div>
                      ))}
                   </div>
                ...
                <div className={`p-12 rounded-[64px] border shadow-2xl ${cardClass}`}>
                   <h3 className="text-xl font-black mb-10 text-slate-400 uppercase tracking-widest">Hardware Ecosystem</h3>
                   <div className="space-y-12">
                      <div className="flex items-center gap-6">
                        <div className="text-4xl">💻</div>
                        <div><div className="font-black text-2xl">Workstations</div><div className="text-slate-400 font-bold uppercase text-[10px]">68% Utilization</div></div>
                      </div>
                      <div className="flex items-center gap-6">
                        <div className="text-4xl">📱</div>
                        <div><div className="font-black text-2xl">Mobile Devices</div><div className="text-slate-400 font-bold uppercase text-[10px]">26% Utilization</div></div>
                      </div>
                      <div className="flex items-center gap-6">
                        <div className="text-4xl">⌚</div>
                        <div><div className="font-black text-2xl">Wearables/Misc</div><div className="text-slate-400 font-bold uppercase text-[10px]">6% Utilization</div></div>
                      </div>
                   </div>
                </div>
             </div>
